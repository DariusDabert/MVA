...
for epoch in range(epochs):
    ...
    for i in range(0, n_train, batch_size):
        ...
        x_batch = torch.stack([torch.LongTensor(X_train[j]) for j in idx[i:i+batch_size]]).to(device)
        y_batch = torch.stack([torch.tensor(y_train[j]) for j in idx[i:i+batch_size]]).to(device)
