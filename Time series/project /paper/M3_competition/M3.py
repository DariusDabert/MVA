...
for index, row in Monthly[0:].iterrows():
    try:
        ...
        t0 = time.time()
        f_abba = forecaster(train, model=batchless_VanillaLSTM_pytorch(lag=5), abba=abba)
        f_abba.train(patience=patience, max_epoch=10000+patience)
        forecast1 = f_abba.forecast(len(test)).tolist()
        t1 = time.time()
        ...
        t0 = time.time()
        f = forecaster(train, model=batchless_VanillaLSTM_pytorch(lag=10), abba=None)
        ...
        ax1.plot(train, 'k')
        #ax1.plot(f_abba.ABBA_representation_numerical, 'k', alpha=0.5)
