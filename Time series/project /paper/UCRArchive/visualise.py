...
for dataset in datasets:
    ...
    if abbamode == 'ABBA':
        abba = ABBA(tol=0.1, max_k=15, verbose=0)
        string, centers = abba.transform(train)
        abba_numerical = abba.inverse_transform(string, centers, train[0])
    else:
        abba = fABBA(tol=0.1, alpha=0.1, sorting='2-norm', scl=1, verbose=0)
        string = abba.fit_transform(train)
        abba_numerical = abba.inverse_transform(string, train[0])
