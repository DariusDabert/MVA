...
def test_against_nb_parameters():
    ...
    if not os.path.isfile('./time/ABBA_LSTM_results_M3.csv'):
        with open('./time/ABBA_LSTM_results_M3.csv', 'a', newline='') as csvfile:
            writer = csv.writer(csvfile, delimiter=' ',
                                quotechar='|', quoting=csv.QUOTE_MINIMAL)
            writer.writerow(header)

    if not os.path.isfile('./time/LSTM_results_M3.csv'):
        with open('./time/LSTM_results_M3.csv', 'a', newline='') as csvfile:
            writer = csv.writer(csvfile, delimiter=' ',
                                quotechar='|', quoting=csv.QUOTE_MINIMAL)
            writer.writerow(header)

    import pandas as pd
    xls = pd.ExcelFile('../../M3_competition/M3C.xls')
    ...
    for index, row in Monthly[0:].iterrows():
        ...
        if True:
            ...
            t0 = time.time()
            f = forecaster(train, model=batchless_VanillaLSTM_pytorch(lag=best_hyperparameters[0], cells_per_layer=50), abba=abba)
            ...
            t0 = time.time()
            f = forecaster(train, model=batchless_VanillaLSTM_pytorch(lag=best_hyperparameter, cells_per_layer=10), abba=None)
            f.train(patience=patience, max_epoch=max_epoch)
            ...
            with open('./time/LSTM_results_M3.csv', 'a', newline='') as csvfile:
                ...
                plt.legend(loc=6)
                plt.savefig('./time/plots/M3_' +str(index)+'.pdf')
                plt.close()
