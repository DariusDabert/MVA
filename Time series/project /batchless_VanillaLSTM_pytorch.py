...
class batchless_VanillaLSTM_pytorch(object):
    ...
    def train(self, patience=10, max_epoch=1000, acceptable_loss=np.inf, batch_size=1, compute_accuracy=False, weight_restarts=False, debug=False):
        """
            ...
            for iter in range(max_epoch):
                ...
                for r in rint: # run through groups
                    ...
                    for i in self.index[r]: # run through group
                        ...
                        # calculate loss
                        if self.features == 1:
                            epoch_accuracy += (y_pred.view(-1, 1).argmax() == self.sequence[i+self.lag, :, :].argmax())  ### our contribution
                            self.loss = self.loss_fn(y_pred.view(-1, 1), self.sequence[i+self.lag, :, :])
                        else:
                            epoch_accuracy += (y_pred.argmax() == self.sequence[i+self.lag, :, :].argmax()) ### our contribution
                ...
                if vec_loss[iter] >= min_loss:
                    ...
                    min_loss_ind = iter
                nb_item = np.sum([len(self.index[i]) for i in range(self.num_augs)])
                epoch_accuracies.append(epoch_accuracy/nb_item)
    ...
    def forecast(self, k, randomize=False, debug=False):
        ...
        for ind in range(len(self.sequence), len(self.sequence) + k):
            ...
            if self.features != 1:
                ...
                p = np.array(p)
                p /= p.sum()
                if randomize: ### our contribution
                    proba = random.random()
                    if proba < 0.05:
                        # choose second most likely
                        idx = np.argsort(list(p))[1]
                    else:
