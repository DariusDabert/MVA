...
from fABBA import fABBA as fABBA
import numpy as np
...
class forecaster(object):
    ...
    def __init__(self, time_series, model, abba=None):
        ...
        elif isinstance(self.abba, fABBA):
            self.pieces = abba.compress(self.normalised_time_series)
            self.ABBA_string, self.parameters = self.abba.digitize(self.pieces)
            # Apply inverse transform.
            self.ABBA_numeric = self.mean + np.dot(self.std, self.abba.inverse_transform(self.ABBA_string, self.normalised_time_series[0]))

            # One hot encode symbolic representation. Create list of all symbols
            # in case symbol does not occur in symbolic representation. (example:
            # flat line and insist k>1)
            self.alphabet = sorted([chr(97+i) for i in range(len(set(self.ABBA_string)))])
            self.sequence = np.array([[0 if char != letter else 1 for char in self.alphabet] for letter in self.ABBA_string])
        else:
    ...
    def train(self, patience=100, max_epoch=100000, acceptable_loss=np.inf, batch_size=1, compute_accuracy=False):
        ### OUR CONTRIBUTION
        if compute_accuracy:
            return self.model_class.train(patience=patience, max_epoch=max_epoch, acceptable_loss=acceptable_loss, batch_size=batch_size, compute_accuracy=compute_accuracy)
        else:
            self.model_class.train(patience=patience, max_epoch=max_epoch, acceptable_loss=acceptable_loss, batch_size=batch_size)

    def forecast(self, k, randomize= False):

        prediction = self.model_class.forecast(k, randomize=randomize)
        ...
        ### OUR CONTRIBUTION (does not work yet)
        if isinstance(self.abba, fABBA):
            s = ''
            for piece in prediction[-k:]:
                idx = np.argmax(piece, axis = 0)
                s += self.alphabet[idx]

            prediction =  np.dot(1, self.abba.inverse_transform(s, self.normalised_time_series[-1]))

            return prediction[1:k+1]
